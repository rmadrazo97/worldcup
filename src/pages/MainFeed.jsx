...
import { getGroups, getMatches } from '../api/scores.js'
import { computeStandings, TEAMS, VENUES } from '../api/mock-data.js'
...
export default function MainFeed() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    Promise.all([getMatches(), getGroups()])
      .then(([m, g]) => {
        if (!alive) return
        setMatches(m)
        setGroups(g)
  ...
  const todayMatchday = useMemo(() => {
    const m = matches.find((x) => x.date === todayIso)
    return m ? m.md : null
  }, [matches, todayIso])
  ...
  const counts = useMemo(() => ({
    ...
    today:     matches.filter(m => m.date === todayIso).length,
    yesterday: matches.filter(m => m.date === yesterdayIso).length,
    upcoming:  matches.filter(m => m.status === "SCHED").length,
  }), [matches, todayIso, yesterdayIso])

  const filtered = useMemo(() => applyQuery(matches, groups, query), [matches, groups, query])
  const isSearching = query.trim().length > 0

  const feedByTab = useMemo(() => {
    let pool = filtered.matches
    if (tab === "live")      pool = pool.filter(m => m.status === "LIVE")
    else if (tab === "today")pool = pool.filter(m => m.date === todayIso)
    else if (tab === "yesterday") pool = pool.filter(m => m.date === yesterdayIso)
    ...
    <div className="app">
      ...
      <div className="shell">
        ...
        {error && matches.length === 0 ? (
          ...
          <>
            {/* Live now */}
            {!isSearching && tab === "all" && <LiveSection matches={matches} />}
            ...
            <section className="section">
              <div className="section-head">
                <h2 className="section-title">
                  ...
                  <span className="badge-count">{tab === "all" && !isSearching
                    ? matches.filter(m => m.date === todayIso).length
              ...
              {(() => {
                const list = (tab === "all" && !isSearching)
                  ? matches.filter(m => m.date === todayIso)
                              .sort((a, b) => {
                                const order = { LIVE: 0, SCHED: 1, FT: 2 }
                                return order[a.status] - order[b.status]
                              })
